...
#define MaxPoint 
*/
int reconstructRoom(std::string path, std::string outName) {
    ...
    Vector3f max_point = ;
    */
    // 512^3 voxels like suggested in the paper, Init the volume with plausible range!!! These are random values which I belive contains all the first frame

    Volume volum(Vector3d{ -1.5, -1.0, -0.1 }, Vector3d{ 1.5, 1.0, 3.5 }, 512, 512, 512, 3);
    volum.zeroOutMemory();
    std::cout<<volum.vol[512]<<std::endl;
    Vector3f distanceBetweenVoxels((volum.max[0] - volum.min[0]) / 511,
        (volum.max[1] - volum.min[1]) / 511,
        (volum.max[2] - volum.min[2]) / 511);
    std::cout << distanceBetweenVoxels;
    // Load video
    ...
    */
    while (sensor.processNextFrame() && iter <= iMax) {
        ...
        */
        ...
        CUDA::SurfaceReconstruction(volum.min,volum.max, volum.weights, volum.vol, currentCameraPose, transMatrixcur, depthMap,source.getNormals(), camparams);
        std::cout << *std::max_element(volum.vol, volum.vol + 512*512*512) << std::endl;
        target = source;
